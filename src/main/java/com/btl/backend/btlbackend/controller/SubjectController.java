...
import com.btl.backend.btlbackend.service.SubjectService;
...
public class SubjectController extends BaseController<SubjectDTO, SubjectService> {
    ...
    @Override
    public ResponseEntity<ResponseMsg> create(@RequestBody SubjectDTO dto) {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Authorization", value = "Authorization token",
                    required = true, dataType = "string", paramType = "header")})
    @Override
    public ResponseEntity<ResponseMsg> update(@PathVariable("id") Long id, @RequestBody SubjectDTO dto) {
